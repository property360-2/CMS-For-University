...
def website_editor(request, website_id):
    ...
    from .section_templates import get_all_templates
    ...
    return render(request, 'websites/editor_v2.html', context)
...
def delete_page(request, website_id, page_id):
    ...
    if website.pages.count() <= 1:
        return JsonResponse({'success': False, 'error': 'Cannot delete the last page'}, status=400)
...
def create_section(request, page_id):
    ...
    try:
        ...
        template = get_template(template_type) if template_type != 'custom' else None
        
        section = Section.objects.create(
            page=page,
            heading=data.get('heading', template['name'] if template else ''),
            body=data.get('body', template['html'] if template else ''),
            template_type=template_type,
            order=max_order + 1,
            # Styling from template or defaults
...
def update_section(request, section_id):
    ...
    try:
        ...
        if 'background_gradient' in data:
            section.background_gradient = data['background_gradient']
        if 'text_color' in data:
            section.text_color = data['text_color']
        if 'font_family' in data:
            section.font_family = data['font_family']
        if 'font_size' in data:
            section.font_size = data['font_size']
        ...
        if 'button_color' in data:
            section.button_color = data['button_color']
        if 'button_text_color' in data:
            section.button_text_color = data['button_text_color']
        ...
        if 'border_radius' in data:
            section.border_radius = data['border_radius']
...
@login_required
@require_http_methods(["POST"])
def upload_image(request, section_id):
...
@login_required
@require_http_methods(["POST"])
def upload_background(request, section_id):
    ...
    try:
        if 'background' in request.FILES:
            section.background_image = request.FILES['background']
            section.save()
            
            return JsonResponse({
                'success': True,
                'background_url': section.background_image.url
            })
        else:
            return JsonResponse({'success': False, 'error': 'No image provided'}, status=400)
    except Exception as e:
